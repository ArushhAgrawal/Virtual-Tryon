...
class CatVTONPipeline:
    def __init__(
        ...
        self.noise_scheduler = DDIMScheduler.from_pretrained(base_ckpt, subfolder="scheduler",local_files_only=True)
        self.vae = AutoencoderKL.from_pretrained("stabilityai/sd-vae-ft-mse",local_files_only=True).to(device, dtype=weight_dtype)
        if not skip_safety_check:
            self.feature_extractor = CLIPImageProcessor.from_pretrained(base_ckpt, subfolder="feature_extractor",local_files_only=True)
            self.safety_checker = StableDiffusionSafetyChecker.from_pretrained(base_ckpt, subfolder="safety_checker",local_files_only=True).to(device, dtype=weight_dtype)
        self.unet = UNet2DConditionModel.from_pretrained(base_ckpt, subfolder="unet",local_files_only=True).to(device, dtype=weight_dtype)
